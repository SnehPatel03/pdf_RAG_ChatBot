...
const ChatPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-[85vh] w-full bg-slate-800 py-1 text-slate-200 overflow-hidden">
      <div className="flex-1 min-h-0 p-4 space-y-4 overflow-y-auto scrollbar-hide">
        {messages.length === 0 ? (
          ...
          messages.map((msg, index) => (
            ...
            >
              <div
                className={`max-w-[85%] rounded-2xl px-4 py-1 shadow-lg ${
                  msg.role === "user"
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="max-w-[85%] rounded-2xl px-4 py-3 shadow-lg bg-slate-700 text-slate-200">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-slate-400 text-sm">Assistant is thinking...</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="flex-shrink-0 p-4 border-t border-slate-700 bg-slate-800 fixed bottom-0 over">
        <div className="flex items-end space-x-3">
          <div className="flex items-center justify-center gap-7 mt-3">
            <textarea
              ...
              className="w-[53vw] px-4 py-2 rounded-xl bg-slate-700 text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500  min-h-[40px] max-h-33 overflow-y-auto scrollbar-hide font-semibold" 
              rows={1}
              disabled={isLoading}
              onKeyDown={(e) => {
